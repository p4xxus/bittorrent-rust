//! Contains types and methods for making a tracker request and deserializing the response.

use self::{peers::PeerConnections, peers6::Peer6Connections};
...
/// helper function to percent-encode bytes
...
pub struct TrackerResponse {
    ...
    pub interval: usize,
    /// peer addresses (compact (IPv4)/non-compact (IPv4 + IPv6))
    peers: PeerConnections,
    /// peer addresses IPv6 (compact)
    peers6: Option<Peer6Connections>,
}
impl TrackerResponse {
    /// gets all the ipv4 and ipv6 addresses from a response
